...
class tools:
    ...
    def loadStockData(stockDict, configManager, proxyServer=None):
        curr = datetime.datetime.now(pytz.timezone('Asia/Kolkata'))
        openTime = curr.replace(hour=9, minute=15)
        last_cached_date = datetime.date.today() #+datetime.timedelta(days=1)  # for monday to friday after 3:30
        weekday = datetime.date.today().weekday()
        if curr < openTime:  # for monday to friday before 9:15
        ...
        local_cashe_file =ConfigManager.StopFolder+"stock_data_" +str(last_cached_date) + ".pkl"
        if os.path.exists(local_cashe_file):
            with open(local_cashe_file, 'rb') as f:
                try:

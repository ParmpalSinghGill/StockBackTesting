...
class SRChannels:
    ...
    def get_sr_vals(self, ind):
        ...
        for cpp in self.pivot_vals:
            width = hi - cpp if cpp <= hi else cpp - lo
    ...
    def changeit(self,x, y):
        # Swap the first part of the pair (index 2*y and 2*x)
        tmp = self.support_resistance[y * 2]
        self.support_resistance[y * 2] = self.support_resistance[x * 2]
        self.support_resistance[x * 2] = tmp

        # Swap the second part of the pair (index 2*y+1 and 2*x+1)
        tmp = self.support_resistance[y * 2 + 1]
        self.support_resistance[y * 2 + 1] = self.support_resistance[x * 2 + 1]
        self.support_resistance[x * 2 + 1] = tmp

    def removeOldPivots(self, ph,pl):
        if any(ph>0) or any(pl>0):  # If there is a pivot high (ph) or pivot low (pl)

            pivotsires = pd.Series(np.where(ph > pl, ph, pl), index=ph.index)  # Keep the original index
            return pivotsires[-self.loopback:]

    # def process_pivots(self,indx pivot_high,pivot_low):
    ...
    def getSupportAndRessitent(self,df):
        ...
        latestpivot=self.removeOldPivots(pivot_high, pivot_low)
